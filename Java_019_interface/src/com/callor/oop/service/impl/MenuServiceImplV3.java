...
public class MenuServiceImplV3 implements MenuService{
	 ...
	 * 사용자가 메뉴를 보고 선택을 했을 때, 선택된 결과 값을 호출한 곳에
	 * 		return; 해줘야한다.
	 * 
	 * 이렇게 설계를 변경하려면 method를 void type이 아닌 int나 String 등 다른 type으로
	 * 		변경해야한다.
	 * 클래스 method를 변경하려면 interface부터 변경하고
	 * 		interface를 implement 한 모든 클래스를 다 변경해야 한다.
	 * 
	 * interface, class의 원형을 변경하지 않고 현재 method에만 어떤 기능을 추가하고자 한다.
	 * 
	 * 별도로 menuItem 변수를 하나 선언하고
	 * 이 변수값으로 선택된 menu를 알 수 있도록 하겠다.
	 */
	 ...
	 * "menuItem"에 적절한 값을 담고 return을 수행하도록 코드 구현했다
	 * 
