...
public class StringArray_02 {
	...
	public static void main(String[] args) {
		 ...
		 * 원래문자열.split(구분자문자열)
		 * 
		 * 원래 문자열을 구분자문자열을 기준으로 나누고 문자열 배열로 만들어 return한다.
		 */
		// 문자열을 " "이 있을때 분해하는 method
		...
		/*
		 * strCart에 담긴 문자열을 글자단위로 잘라서 배열에 담아달라
		 */
		...
		// format method 문자열로만들어서 변수로 담을수있게 만든?
		// 지정된 형식(폼)으로 문자열을 만들어서 return해주는 method
		strCart = String.format("%d:%d:%d", 10,20,30);
		System.out.println(strCart);
		
		// strCart변수에 담긴 문자열중에 20이라는 문자열이 있으면 대한민국 이라는 문자열로 바꾸어서 return
