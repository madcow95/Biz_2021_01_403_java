 ...
 * 2. javac를 사용하여 컴파일을 수행하여
 * 	  Hello.class를 만드는 과정에서
 ...
 * - 클래스 이름과 피일이름은 항상 같아야한다.
 * - 클래스가 HelloKorea이면 파일이름은 HelloKorea.java로 작성을 한다.
 * - 클래스 이름은 첫글자가 반드시 영문대문자
 * - 영문 대소문자, 숫자, _(Under Bar or Under Score)
 * 
 * - 두 개이상의 단어를 조합할때는 Camel Case로 한다
 * 	 Camel Case : 'H'ello'K'orea 처럼 띄어쓰기를 안하는 대신 각 단어 앞의 글자를 영대문자로
 * 
 */
public class HelloKorea {
	 ...
	 */
	// 명령문의 시작부분에 //를 붙이면 명령문은 없는 것과 같다.
	...
	public static void main(String args[]) {
		 ...
		 * 
		 * - 산술(연산)식
		 * 1. 숫자 산술식 : 수학의 4칙연산을 수행하고 결과를 출력한다.
		 * 
		 * 2. 문자열 산술식 : ""로 묶인 문자열을 + 연산하면 문자열을 연결하여 출력
		 * 		문자열 산술식은 + 연산만 가능하다.
		 * 
		 * 3. 문자열 + 숫자 산술식 : 모든 숫자를 자동으로 문자열화하여 문자열 산술식으로 바꾸어 연산하고
		 * 	  결과를 출력한다.
		 * 
		 */
